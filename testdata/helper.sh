...
REPO_LOCATION="joakimkennedy/gore-test"
...
function fetch() {
    if [ -f "/Volumes/Keybase/public/$REPO_LOCATION/latest" ]; then
        LATEST_TAR=$(cat $REPO_LOCATION/latest)
        tar xfvj $REPO_LOCATION/$LATEST_TAR -C $TESTDATA_FOLDER
    elif [ -f "/keybase/public/$REPO_LOCATION/latest" ]; then
        LATEST_TAR=$(cat $REPO_LOCATION/latest)
        tar xfvj $REPO_LOCATION/$LATEST_TAR -C $TESTDATA_FOLDER
    else
...
function upload_and_clean() {
    ...
    if [ -f "/Volumes/Keybase/public/$REPO_LOCATION/latest" ]; then
        REPO_PATH="/Volumes/Keybase/public/$REPO_LOCATION"
    else
        REPO_PATH="/keybase/public/$REPO_LOCATION"
    fi
    
    cp -v $NEW_TAR $REPO_PATH/.
    echo "Removing archive"
    rm $NEW_TAR
    echo "Update latest file record"
    echo "$NEW_TAR" > $REPO_PATH/latest
}
