...
func typeDef(b *bytes.Buffer, st reflect.Type, bits int) {
	...
	for i := 0; i < st.NumField(); i++ {
		...
		case reflect.Uintptr:
			fmt.Fprintf(b, "%s %s\n", fieldName, typeName)
		case reflect.String:
			fmt.Fprintf(b, "%s, %[1]slen %s\n", fieldName, typeName)
		case reflect.Pointer:
			fmt.Fprintf(b, "%s %s\n", fieldName, typeName)
		case reflect.Slice:
			fmt.Fprintf(b, "%s, %[1]slen, %[1]scap %s\n", fieldName, typeName)
...
func toModuledata(b *bytes.Buffer, st reflect.Type, bits int) {
	fmt.Fprintf(b, "func (md %s%d) toModuledata() moduledata {\n", st.Name(), bits)
	fmt.Fprint(b, "return moduledata{\n")
...
func modFieldE(b *bytes.Buffer, st reflect.Type, bits int, modName, parsedName string) {
	...
	if bits == 32 {
		fmt.Fprintf(b, "%sAddr: uint64(md.%[3]s),\n%[1]sLen: uint64(md.%s - md.%s),\n", modName, endName, parsedName)
	} else {
		fmt.Fprintf(b, "%sAddr: md.%[3]s,\n%[1]sLen: md.%s - md.%s,\n", modName, endName, parsedName)
	}
}
...
func modFieldLen(b *bytes.Buffer, st reflect.Type, bits int, modName, parsedName string) {
	...
	if bits == 32 {
		fmt.Fprintf(b, "%sAddr: uint64(md.%s),\n%[1]sLen: uint64(md.%[3]s),\n", modName, parsedName, lenName)
	} else {
		fmt.Fprintf(b, "%sAddr: md.%s,\n%[1]sLen: md.%[3]s,\n", modName, parsedName, lenName)
	}
}
...
func main() {
	...
	fmted := check1(format.Source(b.Bytes()))
	check(os.WriteFile("moduledata_gen.go", fmted, 0o666))
}
